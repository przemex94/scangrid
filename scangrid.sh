...
mini_tcp_scan_run() {
    ...
    screen -S $projectname+_minitcp -dm bash -c "nmap -p- -A -oA /srv/scangrid/minitcp/$id --stats-every 1s --script=all --version-all --osscan-guess --unpriviledged"
    exit 1
}   
...
udp_scan_run() {
    ...
    screen -S $projectname -dm sh -c "nmap -p- -A -oA /srv/scangrid/udp/$id --stats-every 1s --script=all --version-all --osscan-guess --privileged -Pn -T4 -vvv --reason -sU | tee udplog.log"
    exit 1
}
